import type { Metadata } from "next";
import Image from "next/image";
import Link from "next/link";
import { notFound } from "next/navigation";
import {
  AlertTriangle,
  ArrowLeft,
  ArrowRight,
  Calculator,
  Check,
  ClipboardList,
  FileText,
  Phone,
} from "lucide-react";

import { SiteFooter } from "@/components/site-footer";
import { SiteHeader } from "@/components/site-header";
import {
  constructionPricingArticles,
  getConstructionPricingArticle,
} from "@/data/construction-pricing-articles";

const listingHref = "/bao-gia/thiet-ke-kien-truc-va-xay-dung-tron-goi";

export function generateStaticParams() {
  return constructionPricingArticles.map(({ slug }) => ({ slug }));
}

export async function generateMetadata({
  params,
}: PageProps<
  "/bao-gia/thiet-ke-kien-truc-va-xay-dung-tron-goi/[slug]"
>): Promise<Metadata> {
  const { slug } = await params;
  const article = getConstructionPricingArticle(slug);

  if (!article) {
    return { title: "Bài viết không tồn tại | Tổ Ấm Hoàn Hảo" };
  }

  return {
    title: `${article.title} | Tổ Ấm Hoàn Hảo`,
    description: article.excerpt,
  };
}

export default async function ConstructionPricingArticlePage({
  params,
}: PageProps<"/bao-gia/thiet-ke-kien-truc-va-xay-dung-tron-goi/[slug]">) {
  const { slug } = await params;
  const article = getConstructionPricingArticle(slug);

  if (!article) {
    notFound();
  }

  const contentSections = article.contentSections ??
    article.sections.map((section) => ({
      title: section.title,
      blocks: section.paragraphs.map((text) => ({
        type: text.startsWith("• ") ? ("bullet" as const) : ("paragraph" as const),
        text: text.replace(/^• /, ""),
      })),
    }));

  const relatedArticles = constructionPricingArticles
    .filter((item) => item.slug !== article.slug)
    .slice(0, 3);

  return (
    <main className="min-h-screen bg-[#f8f3ec] text-[#2d271f]">
      <section className="relative overflow-hidden border-b border-[#e1d6c7]">
        <SiteHeader />
        <div className="mx-auto grid min-h-[600px] max-w-[1320px] pt-20 lg:grid-cols-[0.92fr_1.08fr] xl:pt-[120px]">
          <div className="relative z-10 flex items-center px-6 py-16 sm:px-10 lg:px-8 lg:py-12">
            <div className="max-w-[600px]">
              <nav
                aria-label="Điều hướng trang"
                className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-[#766d60]"
              >
                <Link href="/" className="transition hover:text-[#9a733e]">
                  Trang chủ
                </Link>
                <span>/</span>
                <Link
                  href={listingHref}
                  className="transition hover:text-[#9a733e]"
                >
                  Báo giá xây dựng
                </Link>
                <span>/</span>
                <span className="line-clamp-1">{article.title}</span>
              </nav>

              <p className="mt-10 text-xs font-bold uppercase tracking-[0.18em] text-[#9a733e]">
                Báo giá xây dựng
              </p>
              <h1 className="mt-4 font-serif text-4xl leading-[1.08] text-[#1f1a13] sm:text-5xl lg:text-[3.5rem]">
                {article.title}
              </h1>
              <p className="mt-6 max-w-[550px] text-base leading-8 text-[#584f43]">
                {article.excerpt}
              </p>

              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  href="/lien-he"
                  className="inline-flex min-h-12 items-center gap-2 bg-[#777b61] px-6 text-xs font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#62674f]"
                >
                  Nhận dự toán mới
                  <ArrowRight aria-hidden="true" className="h-4 w-4" />
                </Link>
                <a
                  href="tel:0903897555"
                  className="inline-flex min-h-12 items-center gap-2 border border-[#b9a689] px-6 text-xs font-bold uppercase tracking-[0.08em] text-[#5b4932] transition hover:bg-[#eee3d5]"
                >
                  <Phone aria-hidden="true" className="h-4 w-4" />
                  0903.897.555
                </a>
              </div>
            </div>
          </div>

          <div className="relative min-h-[340px] lg:min-h-full">
            <Image
              src={article.image}
              alt={article.title}
              fill
              priority
              sizes="(min-width: 1024px) 54vw, 100vw"
              className="object-cover"
            />
            <div className="absolute inset-y-0 left-0 hidden w-28 bg-gradient-to-r from-[#f8f3ec] to-transparent lg:block" />
          </div>
        </div>
      </section>

      <section className="border-b border-[#e1d6c7] bg-[#fdfaf6]">
        <div className="mx-auto grid max-w-[1320px] sm:grid-cols-2 lg:grid-cols-3">
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 sm:border-r lg:px-8">
            <FileText
              aria-hidden="true"
              className="h-7 w-7 text-[#a0783e]"
              strokeWidth={1.2}
            />
            <p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-[#8c795f]">
              Nhóm nội dung
            </p>
            <p className="mt-2 text-sm font-semibold leading-6 text-[#3e352a]">
              Báo giá thiết kế kiến trúc và xây dựng
            </p>
          </article>
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 lg:border-r lg:px-8">
            <ClipboardList
              aria-hidden="true"
              className="h-7 w-7 text-[#a0783e]"
              strokeWidth={1.2}
            />
            <p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-[#8c795f]">
              Phạm vi
            </p>
            <p className="mt-2 text-sm font-semibold leading-6 text-[#3e352a]">
              Nhà phố, phần thô và xây dựng trọn gói
            </p>
          </article>
          <article className="px-6 py-7 lg:px-8">
            <Calculator
              aria-hidden="true"
              className="h-7 w-7 text-[#a0783e]"
              strokeWidth={1.2}
            />
            <p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-[#8c795f]">
              Cơ sở lập giá
            </p>
            <p className="mt-2 text-sm font-semibold leading-6 text-[#3e352a]">
              Hồ sơ thiết kế, điều kiện khu đất và vật liệu thực tế
            </p>
          </article>
        </div>
      </section>

      <div className="mx-auto max-w-[1320px] px-5 pt-10 sm:px-8">
        <div className="flex gap-4 border border-[#d8c4a5] bg-[#f3e7d4] p-5 text-sm leading-7 text-[#66533b]">
          <AlertTriangle
            aria-hidden="true"
            className="mt-1 h-5 w-5 shrink-0 text-[#a0783e]"
            strokeWidth={1.5}
          />
          <p>
            Nội dung được lưu từ website cũ, gồm các mốc giá và ví dụ theo thời
            điểm xuất bản. Đây không phải báo giá hiện hành; dự toán mới cần được
            khảo sát và lập theo vật liệu, điều kiện thực tế.
          </p>
        </div>
      </div>

      <div className="mx-auto grid max-w-[1320px] gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_330px] lg:py-24">
        <article className="min-w-0">
          <section className="border-b border-[#ded2c1] pb-12">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#9a733e]">
              Nội dung chính
            </p>
            <h2 className="mt-4 font-serif text-4xl leading-tight text-[#30291f] sm:text-5xl">
              Nội dung và phạm vi cần lưu ý
            </h2>
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {[
                "Cách tính và hạng mục cần chuẩn bị theo nội dung bài viết gốc.",
                "Số liệu cũ chỉ dùng để tham khảo, không thay cho dự toán hiện tại.",
                "Cần có hồ sơ thiết kế và khảo sát thực tế trước khi chốt ngân sách.",
                "Vật liệu, kết cấu và điều kiện thi công quyết định chi phí cuối cùng.",
              ].map((item) => (
                <p
                  key={item}
                  className="flex gap-3 border border-[#ded2c1] bg-[#fdfaf6] p-4 text-sm leading-6 text-[#62594d]"
                >
                  <Check
                    aria-hidden="true"
                    className="mt-0.5 h-4 w-4 shrink-0 text-[#8a7650]"
                    strokeWidth={2.4}
                  />
                  {item}
                </p>
              ))}
            </div>
          </section>

          <div className="divide-y divide-[#ded2c1]">
            {contentSections.map((section, sectionIndex) => (
              <section key={section.title} className="py-12">
                <p className="text-xs font-bold text-[#a0783e]">
                  {String(sectionIndex + 1).padStart(2, "0")}
                </p>
                <h2 className="mt-3 font-serif text-3xl leading-tight text-[#30291f] sm:text-4xl">
                  {section.title}
                </h2>
                <div className="mt-6 space-y-5">
                  {section.blocks.map((block, blockIndex) => {
                    if (block.type === "image") {
                      return (
                        <figure
                          key={`${block.src}-${blockIndex}`}
                          className="overflow-hidden border border-[#ded2c1] bg-white"
                        >
                          <Image
                            src={block.src}
                            alt={`${article.title} - hình ảnh minh họa ${blockIndex + 1}`}
                            width={1400}
                            height={875}
                            sizes="(min-width: 1024px) 62vw, 100vw"
                            className="h-auto w-full object-contain"
                          />
                        </figure>
                      );
                    }

                    return block.type === "bullet" ? (
                      <p
                        key={`${block.text}-${blockIndex}`}
                        className="flex gap-3 text-base leading-8 text-[#62594d]"
                      >
                        <Check
                          aria-hidden="true"
                          className="mt-2 h-4 w-4 shrink-0 text-[#9a733e]"
                          strokeWidth={2}
                        />
                        <span>{block.text}</span>
                      </p>
                    ) : (
                      <p
                        key={`${block.text}-${blockIndex}`}
                        className="text-base leading-8 text-[#62594d]"
                      >
                        {block.text}
                      </p>
                    );
                  })}
                </div>
              </section>
            ))}
          </div>

          <div className="mt-10 flex flex-wrap gap-3">
            <Link
              href={listingHref}
              className="inline-flex items-center gap-2 text-sm font-bold text-[#70532b] transition hover:text-[#9a733e]"
            >
              <ArrowLeft aria-hidden="true" className="h-4 w-4" />
              Quay lại báo giá xây dựng
            </Link>
          </div>
        </article>

        <aside className="h-fit border border-[#ded2c1] bg-[#fdfaf6] p-6 lg:sticky lg:top-28">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#9a733e]">
            Cần dự toán mới?
          </p>
          <h2 className="mt-3 font-serif text-3xl leading-tight text-[#30291f]">
            Nhận tư vấn theo công trình của bạn
          </h2>
          <p className="mt-4 text-sm leading-7 text-[#62594d]">
            Gửi diện tích, vị trí và nhu cầu. Đội ngũ sẽ hỗ trợ rà soát phạm vi
            và chuẩn bị dự toán phù hợp.
          </p>
          <Link
            href="/lien-he"
            className="mt-7 inline-flex w-full min-h-12 items-center justify-center gap-2 bg-[#777b61] px-5 text-xs font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#62674f]"
          >
            Gửi yêu cầu tư vấn
            <ArrowRight aria-hidden="true" className="h-4 w-4" />
          </Link>
          <a
            href="tel:0903897555"
            className="mt-3 inline-flex w-full items-center justify-center gap-2 border border-[#b9a689] px-5 py-3 text-sm font-semibold text-[#5b4932] transition hover:bg-[#eee3d5]"
          >
            <Phone aria-hidden="true" className="h-4 w-4" />
            0903.897.555
          </a>
        </aside>
      </div>

      <section className="border-t border-[#e1d6c7] bg-[#eee5d8] px-5 py-14 sm:px-8">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#9a733e]">
                Xem thêm
              </p>
              <h2 className="mt-2 font-serif text-3xl text-[#30291f]">
                Các bài liên quan
              </h2>
            </div>
            <Link
              href={listingHref}
              className="inline-flex items-center gap-2 text-sm font-bold text-[#70532b]"
            >
              Xem tất cả
              <ArrowRight aria-hidden="true" className="h-4 w-4" />
            </Link>
          </div>
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {relatedArticles.map((item) => (
              <Link
                key={item.slug}
                href={`${listingHref}/${item.slug}`}
                className="group overflow-hidden border border-[#d7c9b6] bg-[#f9f4ed] transition hover:-translate-y-1"
              >
                <div className="relative aspect-[1.65] overflow-hidden">
                  <Image
                    src={item.image}
                    alt={item.title}
                    fill
                    sizes="(min-width: 1024px) 30vw, 100vw"
                    className="object-cover transition duration-500 group-hover:scale-105"
                  />
                </div>
                <div className="p-5">
                  <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#9a733e]">
                    Báo giá xây dựng
                  </p>
                  <h3 className="mt-3 text-base font-bold leading-6 text-[#40362b]">
                    {item.title}
                  </h3>
                  <span className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-[#70532b]">
                    Xem bài viết
                    <ArrowRight aria-hidden="true" className="h-4 w-4" />
                  </span>
                </div>
              </Link>
            ))}
          </div>
        </div>
      </section>

      <SiteFooter />
    </main>
  );
}
