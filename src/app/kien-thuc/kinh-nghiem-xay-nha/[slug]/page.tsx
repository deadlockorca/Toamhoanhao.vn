...
import {
  buildingExperienceArticles,
  buildingExperienceListingHref,
  getBuildingExperienceArticle,
  getBuildingExperienceArticleHref,
} from "@/data/building-experience-articles";

export function generateStaticParams() {
  return buildingExperienceArticles.map(({ slug }) => ({ slug }));
}

export async function generateMetadata({
  params,
}: PageProps<"/kien-thuc/kinh-nghiem-xay-nha/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const article = getBuildingExperienceArticle(slug);
...
export default async function BuildingExperienceArticlePage({
  params,
}: PageProps<"/kien-thuc/kinh-nghiem-xay-nha/[slug]">) {
  const { slug } = await params;
  const article = getBuildingExperienceArticle(slug);
  ...
  const relatedArticles = buildingExperienceArticles
          ...
          <div className="relative z-10 flex items-center px-6 py-16 sm:px-10 lg:px-8 lg:py-12">
            <div className="max-w-[600px]">
              ...
              >
                ...
                <span>/</span>
                <Link
                  href={buildingExperienceListingHref}
      ...
      <section className="border-b border-[#e1d6c7] bg-[#fdfaf6]">
        <div className="mx-auto grid max-w-[1320px] sm:grid-cols-3">
          ...
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 sm:border-r lg:px-8">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-[#3e352a]">
              {article.sections.length} phần nội dung chuyên sâu
            </p>
          </article>
        ...
        <article className="min-w-0">
          <section className="border-b border-[#ded2c1] pb-12">
            ...
            <h2 className="mt-4 font-serif text-4xl leading-tight text-[#30291f] sm:text-5xl">
              Góc nhìn thực tế để chuẩn bị kỹ hơn
            </h2>
            <p className="mt-6 max-w-3xl text-base leading-8 text-[#62594d]">
              {article.excerpt}
            </p>
          </section>
                ...
                <div className="mt-6 space-y-5">
                  {section.blocks.map((block, blockIndex) => {
                    if (block.type === "image") {
                      return (
                        ...
                        >
                          <Image
                            src={block.src}
                            alt={block.alt || `${article.title} - hình ảnh minh họa ${blockIndex + 1}`}
                            width={1400}
                            height={875}
                    ...
                    return block.type === "bullet" ? (
                      ...
                      <p key={`${block.text}-${blockIndex}`} className="text-base leading-8 text-[#62594d]">
                        {block.text}
                      </p>
                    );
                  })}
          ...
          <Link
            href={buildingExperienceListingHref}
            className="mt-10 inline-flex items-center gap-2 text-sm font-bold text-[#70532b] transition hover:text-[#9a733e]"
          >
            <ArrowLeft aria-hidden="true" className="h-4 w-4" />
            Quay lại kinh nghiệm xây nhà
          </Link>
        </article>
            ...
            <ol className="mt-5 space-y-3 text-sm leading-6 text-[#6b6154]">
              {article.sections.map((section, index) => (
                <li key={`${section.title}-${index}`} className="flex gap-3">
                  ...
                  <a href={`#section-${index + 1}`} className="transition hover:text-[#8a6330]">
                    {section.title}
                  </a>
                </li>
              ))}
          ...
          <section className="border border-[#ded2c1] bg-[#f3eade] p-6 lg:p-7">
            ...
            <div className="mt-4 space-y-4">
              {relatedArticles.map((item) => (
                <Link
                  key={item.slug}
                  href={getBuildingExperienceArticleHref(item.slug)}
                  className="group block border-b border-[#ddcfba] pb-4 last:border-b-0 last:pb-0"
                >
                  <p className="text-sm font-semibold leading-6 text-[#4a3f30] transition group-hover:text-[#8a6330]">
                    {item.title}
                  </p>
                  <span className="mt-2 inline-flex items-center gap-2 text-xs font-bold text-[#8a6330]">
                    Đọc bài viết
      ...
      <section className="border-t border-[#e1d6c7] bg-[#eee5d8] px-5 py-14 sm:px-8">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#9a733e]">Xem thêm</p>
              <h2 className="mt-2 font-serif text-3xl text-[#30291f]">Các bài cùng chủ đề</h2>
            </div>
            <Link href={buildingExperienceListingHref} className="inline-flex items-center gap-2 text-sm font-bold text-[#70532b]">
              Xem tất cả
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {relatedArticles.map((item) => (
              ...
              >
                ...
                <div className="p-5">
                  <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#9a733e]">Kinh nghiệm xây nhà</p>
                  <h3 className="mt-3 text-base font-bold leading-6 text-[#40362b]">{item.title}</h3>
