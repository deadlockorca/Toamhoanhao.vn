...
export const metadata: Metadata = {
  ...
  description:
    "Gợi ý thực tế để kiến tạo không gian sống đẹp, tiện nghi và hài hòa với lối sống của gia đình.",
};
...
export default async function BeautifulHomeKnowledgePage({
  ...
  const pageCount = Math.max(
    1,
    Math.ceil(beautifulHomeArticles.length / articlesPerPage),
  );
          ...
          <div className="relative z-10 flex items-end bg-[#f8f3ec]/90 px-6 pb-14 pt-24 sm:px-10 lg:items-center lg:px-8 lg:pb-0">
            <div className="max-w-[525px]">
              ...
              <h1 className="mt-4 font-serif text-5xl leading-[1.04] text-[#1f1a13] sm:text-6xl">
                Kiến thức <em className="text-[#74785f]">nhà đẹp</em>
              </h1>
              <p className="mt-6 max-w-[455px] text-base leading-8 text-[#584f43]">
                Những gợi ý gần gũi để kiến tạo một không gian đẹp, tiện nghi và
                mang dấu ấn riêng của mỗi gia đình.
              </p>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-[1320px]">
          ...
          <div className="mt-9 grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3">
            {visibleArticles.map((article) => (
              <Link
                key={article.slug}
                href={getBeautifulHomeArticleHref(article.slug)}
          ...
          <ArticlePagination
            basePath="/kien-thuc/kien-thuc-nha-dep"
