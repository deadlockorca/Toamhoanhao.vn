  ...
  return (
    <main className="min-h-screen bg-[#f8f3ec] text-[#2c261e]">
      ...
      <div className="mx-auto grid max-w-[1320px] gap-8 px-5 py-14 sm:px-8 lg:grid-cols-[minmax(0,1fr)_260px] lg:py-20"><div className="min-w-0 space-y-16">
        ...
        <section><div className="flex flex-wrap items-end justify-between gap-4"><div><SectionTitle>Bài viết về thiết kế nội thất</SectionTitle><p className="mt-2 text-sm text-[#756b5e]">Tổng hợp kinh nghiệm từ chuyên mục cũ của Tổ Ấm Hoàn Hảo.</p></div><span className="text-xs font-bold uppercase tracking-[0.05em] text-[#7b623d]">{interiorDesignExperienceArticles.length} bài viết</span></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{interiorDesignExperienceArticles.map((article) => <Link key={article.slug} href={getInteriorDesignExperienceArticleHref(article.slug)} className="group overflow-hidden border border-[#e0d5c6] bg-[#fdfaf6] transition hover:-translate-y-1"><div className="relative aspect-[1.55]"><Image src={article.image} alt={article.title} fill sizes="(min-width: 1024px) 30vw, 50vw" className="object-cover transition duration-500 group-hover:scale-105" /></div><div className="p-5"><p className="text-xs text-[#8d7c66]">{article.date}</p><h3 className="mt-2 font-serif text-xl leading-tight text-[#332b21]">{article.title}</h3><p className="mt-3 text-xs leading-5 text-[#756b5e]">{article.excerpt}</p><span className="mt-4 inline-block text-xs font-bold uppercase tracking-[0.08em] text-[#8a6330]">Đọc bài viết</span></div></Link>)}</div></section>
      </div><aside className="h-fit space-y-5 lg:sticky lg:top-8"><section className="border border-[#e0d5c6] bg-[#fdfaf6] p-6"><h2 className="text-xs font-bold uppercase tracking-[0.14em] text-[#5c5143]">Mục lục</h2><ol className="mt-5 space-y-3 text-xs leading-5 text-[#74695c]">{["Thiết kế bắt đầu từ đâu?", "Các giai đoạn thiết kế", "Lưu ý quan trọng", "Chi phí và ngân sách", "Sai lầm thường gặp"].map((item, index) => <li key={item} className="flex gap-2"><span>{index + 1}.</span><span>{item}</span></li>)}</ol></section><section className="border border-[#e0d5c6] bg-[#fdfaf6] p-6"><h2 className="text-xs font-bold uppercase tracking-[0.14em] text-[#5c5143]">Bài viết mới</h2><div className="mt-5 space-y-4">{interiorDesignExperienceArticles.slice(0, 5).map((article) => <Link key={article.slug} href={getInteriorDesignExperienceArticleHref(article.slug)} className="flex gap-3 transition hover:opacity-75"><div className="relative h-14 w-16 shrink-0 overflow-hidden"><Image src={article.image} alt={article.title} fill sizes="64px" className="object-cover" /></div><div><h3 className="text-xs font-bold leading-5 text-[#4b4033]">{article.title}</h3><p className="mt-1 text-[11px] text-[#8d7c66]">{article.date}</p></div></Link>)}</div></section><section className="border border-[#e0d5c6] bg-[#f3eade] p-6"><BookOpenCheck aria-hidden="true" className="h-9 w-9 text-[#a0783e]" strokeWidth={1.2} /><h2 className="mt-5 font-serif text-2xl text-[#3b3024]">Cẩm nang nội thất</h2><p className="mt-3 text-xs leading-6 text-[#74695c]">Các kiến thức thực tế về phong cách, công năng, vật liệu và quy trình thiết kế nội thất.</p></section></aside></div>
      <SiteFooter />
    </main>
  );
}
