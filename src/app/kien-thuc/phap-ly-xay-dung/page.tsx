  ...
  return (
    <main className="min-h-screen bg-[#f8f3ec] text-[#2c261e]">
      ...
      <div className="mx-auto grid max-w-[1320px] gap-8 px-5 py-14 sm:px-8 lg:grid-cols-[minmax(0,1fr)_260px] lg:py-20"><div className="min-w-0 space-y-16">
        ...
        <section>
          <div className="flex flex-wrap items-end justify-between gap-4"><div><SectionTitle>Bài viết về pháp lý xây dựng</SectionTitle><p className="mt-2 text-sm text-[#756b5e]">Hai bài viết gốc từ chuyên mục Pháp lý xây dựng của Tổ Ấm Hoàn Hảo.</p></div><span className="text-xs font-bold uppercase tracking-[0.05em] text-[#7b623d]">{buildingLegalArticles.length} bài viết</span></div>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {buildingLegalArticles.map((article) => <Link key={article.slug} href={getBuildingLegalArticleHref(article.slug)} className="group overflow-hidden border border-[#e0d5c6] bg-[#fdfaf6] transition hover:-translate-y-1"><div className="relative aspect-[1.55]"><Image src={article.image} alt={article.title} fill sizes="(min-width: 1024px) 40vw, 100vw" className="object-cover transition duration-500 group-hover:scale-105" /></div><div className="p-5"><p className="text-xs text-[#8d7c66]">{article.date}</p><h3 className="mt-2 font-serif text-xl leading-tight text-[#332b21]">{article.title}</h3><p className="mt-3 text-xs leading-5 text-[#756b5e]">{article.excerpt}</p><span className="mt-4 inline-block text-xs font-bold uppercase tracking-[0.08em] text-[#8a6330]">Đọc bài viết</span></div></Link>)}
          </div>
        </section>
      </div><aside className="h-fit space-y-5 lg:sticky lg:top-8"><section className="border border-[#e0d5c6] bg-[#fdfaf6] p-6"><h2 className="text-xs font-bold uppercase tracking-[0.14em] text-[#5c5143]">Mục lục</h2><ol className="mt-5 space-y-3 text-xs leading-5 text-[#74695c]">{["Pháp lý bắt đầu từ đâu?", "Các đầu việc quan trọng", "Lưu ý khi chuẩn bị thủ tục", "Chi phí và tiến độ", "Sai lầm thường gặp"].map((item, index) => <li key={item} className="flex gap-2"><span>{index + 1}.</span><span>{item}</span></li>)}</ol></section><section className="border border-[#e0d5c6] bg-[#fdfaf6] p-6"><h2 className="text-xs font-bold uppercase tracking-[0.14em] text-[#5c5143]">Bài viết mới</h2><div className="mt-5 space-y-4">{buildingLegalArticles.map((article) => <Link key={article.slug} href={getBuildingLegalArticleHref(article.slug)} className="flex gap-3 transition hover:opacity-75"><div className="relative h-14 w-16 shrink-0 overflow-hidden"><Image src={article.image} alt={article.title} fill sizes="64px" className="object-cover" /></div><div><h3 className="text-xs font-bold leading-5 text-[#4b4033]">{article.title}</h3><p className="mt-1 text-[11px] text-[#8d7c66]">{article.date}</p></div></Link>)}</div></section><section className="border border-[#e0d5c6] bg-[#f3eade] p-6"><Gavel aria-hidden="true" className="h-9 w-9 text-[#a0783e]" strokeWidth={1.2} /><h2 className="mt-5 font-serif text-2xl text-[#3b3024]">Lưu ý quan trọng</h2><p className="mt-3 text-xs leading-6 text-[#74695c]">Thông tin trên trang mang tính tổng quan. Quy định và thủ tục cụ thể cần được kiểm tra theo từng địa phương, thời điểm và hiện trạng công trình.</p></section></aside></div>
      <SiteFooter />
    </main>
  );
}
