...
export default function WorkshopPage() {
  return (
    <main className="min-h-screen bg-[#f8f3ec] text-[#28221a]">
      <section className="relative overflow-hidden border-b border-[#e1d6c7] bg-[#f8f3ec]"><SiteHeader /><div className="mx-auto grid min-h-[610px] max-w-[1320px] pt-20 lg:grid-cols-[0.9fr_1.1fr]"><div className="flex items-end px-6 pb-16 pt-24 sm:px-10 lg:items-center lg:px-8 lg:pb-0"><div className="max-w-[540px]"><nav aria-label="Điều hướng trang" className="text-xs text-[#766d60]"><Link href="/" className="transition hover:text-[#9a733e]">Trang chủ</Link><span className="mx-3">/</span><Link href="/gioi-thieu" className="transition hover:text-[#9a733e]">Giới thiệu</Link><span className="mx-3">/</span><span>Xưởng sản xuất nội thất</span></nav><p className="mt-12 text-xs font-bold uppercase tracking-[0.18em] text-[#9a733e]">Giới thiệu</p><h1 className="mt-4 font-serif text-5xl leading-[1.02] text-[#1f1a13] sm:text-6xl lg:text-7xl">Xưởng sản xuất <em className="text-[#74785f]">nội thất</em></h1><p className="mt-7 max-w-[470px] text-base leading-8 text-[#584f43]">Xưởng sản xuất nội thất hiện đại giúp chúng tôi chủ động kiểm soát chất lượng trong từng công đoạn, mang đến giải pháp phù hợp, bền đẹp và tối ưu chi phí cho mỗi gia đình.</p><div className="mt-9 flex flex-wrap gap-3"><a href="#nang-luc-xuong" className="inline-flex h-11 items-center bg-[#70745d] px-5 text-xs font-bold uppercase tracking-[0.06em] text-white transition hover:bg-[#5d614d]">Tham quan xưởng</a><a href="#tu-van" className="inline-flex h-11 items-center border border-[#cdbda8] px-5 text-xs font-bold uppercase tracking-[0.06em] text-[#6b5231] transition hover:border-[#9a733e] hover:text-[#8a6536]">Nhận tư vấn</a></div></div></div><div className="relative min-h-[380px] lg:min-h-full"><Image src="/images/xuong-san-xuat/banner.png" alt="Không gian xưởng sản xuất nội thất Tổ Ấm Hoàn Hảo" fill priority sizes="(min-width: 1024px) 55vw, 100vw" className="object-cover" /></div></div></section>
      ...
      <section id="tu-van" className="bg-[#6f745e] px-5 py-16 text-white sm:px-8"><div className="mx-auto flex max-w-[1320px] flex-col items-center text-center"><Sparkles aria-hidden="true" className="h-9 w-9 text-white/80" strokeWidth={1.25} /><h2 className="mt-5 max-w-[680px] font-serif text-4xl leading-tight sm:text-5xl">Sẵn sàng hiện thực hóa nội thất cho tổ ấm của bạn?</h2><p className="mt-4 max-w-[570px] text-sm leading-7 text-white/80">Đội ngũ Tổ Ấm Hoàn Hảo luôn sẵn sàng tư vấn và đề xuất giải pháp phù hợp nhất.</p><a href="#" className="mt-8 inline-flex h-11 items-center bg-white px-6 text-xs font-bold uppercase tracking-[0.06em] text-[#657052] transition hover:bg-[#f0e9df]">Đặt lịch tư vấn ngay</a></div></section>
      <SiteFooter />
    </main>
  );
}
