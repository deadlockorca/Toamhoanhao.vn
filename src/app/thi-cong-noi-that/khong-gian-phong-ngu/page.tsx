...
import { ArrowRight, BedDouble, BedSingle, Cpu, Factory, Hammer, Phone, Ruler, Sofa, Sparkles, Wrench } from "lucide-react";

import { ConsultationCta } from "@/components/home/consultation-cta";
import { SiteFooter } from "@/components/site-footer";
import { SiteHeader } from "@/components/site-header";
import { getBedroomSpaceArticleHref, bedroomSpaceArticles } from "@/data/bedroom-space-articles";

export const metadata: Metadata = {
  title: "Không gian phòng ngủ | Tổ Ấm Hoàn Hảo",
  description:
    "Tổng hợp các bài viết về thiết kế, thi công không gian phòng ngủ: phòng ngủ hiện đại, luxury, tân cổ điển, Scandinavian, trẻ em và master.",
};

const services = [
  { icon: BedDouble, title: "Thiết kế phòng ngủ", content: "Tư vấn thiết kế phòng ngủ theo phong cách và nhu cầu sử dụng." },
  { icon: BedSingle, title: "Tủ quần áo, giường ngủ", content: "Thiết kế và sản xuất tủ quần áo, giường ngủ đa dạng kiểu dáng." },
  { icon: Wrench, title: "Thi công trọn gói", content: "Thi công không gian phòng ngủ từ phần thô đến hoàn thiện." },
  { icon: Factory, title: "Sản xuất đồ gỗ", content: "Giường, tủ, bàn trang điểm và đồ gỗ nội thất gia công theo yêu cầu." },
];

export default function BedroomSpacePage() {
  return (
    <main className="min-h-screen bg-[#f8f3ec] text-[#2d271f]">
      <section className="relative overflow-hidden border-b border-[#e1d6c7]">
        ...
        <div className="mx-auto grid min-h-[560px] max-w-[1320px] pt-20 lg:grid-cols-[0.88fr_1.12fr] xl:pt-[120px]">
          <div className="relative z-10 flex items-center px-6 py-16 sm:px-10 lg:px-8 lg:py-12">
            <div className="max-w-[560px]">
              <nav aria-label="Điều hướng trang" className="text-xs text-[#766d60]">
                ...
                <span className="mx-3">/</span>
                <span>Không gian phòng ngủ</span>
              </nav>
              ...
              <h1 className="mt-4 font-serif text-5xl leading-[1.03] text-[#1f1a13] sm:text-6xl">
                Không gian <span className="block italic text-[#7f8169]">phòng ngủ</span>
              </h1>
              <p className="mt-6 max-w-[510px] text-base leading-8 text-[#584f43]">
                Tổng hợp các bài viết, mẫu thiết kế và công trình thi công phòng ngủ tiêu biểu, giúp
          ...
          <div className="relative min-h-[340px] lg:min-h-full">
            <Image src="/images/thi-cong-noi-that/khong-gian-phong-ngu/bai-1/1.webp" alt="Không gian phòng ngủ" fill priority sizes="(min-width: 1024px) 56vw, 100vw" className="object-cover" />
        ...
        <div className="mx-auto max-w-[1320px]">
          <div className="max-w-[930px]">
            ...
            <h2 className="mt-3 font-serif text-4xl leading-tight text-[#30291f] sm:text-5xl">
              Thư viện mẫu thiết kế và thi công không gian phòng ngủ
            </h2>
            <p className="mt-5 max-w-[760px] text-sm leading-7 text-[#6f6558]">
              Tổng hợp các bài viết về không gian phòng ngủ từ website cũ, giúp bạn tham khảo kiểu dáng, chất liệu
              và phong cách trước khi đưa ra quyết định cho công trình của mình.
            </p>
          </div>

          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {bedroomSpaceArticles.map((item, index) => (
              <Link
                key={item.slug}
                href={item.externalHref ?? getBedroomSpaceArticleHref(item.slug)}
      ...
      <section className="border-y border-[#e1d6c7] bg-[#fdfaf6] px-5 py-16 sm:px-8 lg:py-20">
        <div className="mx-auto max-w-[1320px]">
          <div className="text-center">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#9a733e]">Dịch vụ thi công phòng ngủ</p>
            <h2 className="mt-3 font-serif text-4xl text-[#30291f]">Từ thiết kế đến hoàn thiện không gian phòng ngủ</h2>
          </div>
      ...
      <section className="relative overflow-hidden bg-[#3d382d] px-5 py-16 text-white sm:px-8 lg:py-20">
        <div className="relative mx-auto flex max-w-[1120px] flex-col items-center text-center">
          <BedDouble aria-hidden="true" className="h-9 w-9 text-[#e6c897]" strokeWidth={1.25} />
          <h2 className="mt-4 max-w-[760px] font-serif text-4xl leading-tight sm:text-5xl">
            Sẵn sàng kiến tạo không gian phòng ngủ cho tổ ấm của bạn?
          </h2>
          <p className="mt-5 max-w-[680px] text-sm leading-7 text-[#e8e0d5]">
            Liên hệ ngay để được tư vấn và báo giá thiết kế, thi công không gian phòng ngủ phù hợp nhất.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link href="/lien-he" className="inline-flex min-h-12 items-center gap-2 bg-[#858a6c] px-7 text-xs font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#969c78]">
              Yêu cầu tư vấn
              <ArrowRight aria-hidden="true" className="h-4 w-4" />
            </Link>
