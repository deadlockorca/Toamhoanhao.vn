...
import { ArrowLeft, ArrowRight, BedDouble, Check, Phone, Ruler, Sparkles } from "lucide-react";
...
import {
  getBedroomSpaceArticleBySlug,
  getBedroomSpaceArticleHref,
  bedroomSpaceArticles,
} from "@/data/bedroom-space-articles";

export function generateStaticParams() {
  return bedroomSpaceArticles
...
export async function generateMetadata({
  params,
}: PageProps<"/thi-cong-noi-that/khong-gian-phong-ngu/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const article = getBedroomSpaceArticleBySlug(slug);
...
export default async function BedroomSpaceArticlePage({
  params,
}: PageProps<"/thi-cong-noi-that/khong-gian-phong-ngu/[slug]">) {
  const { slug } = await params;
  const article = getBedroomSpaceArticleBySlug(slug);
  ...
  const relatedArticles = article.relatedSlugs
    .map(getBedroomSpaceArticleBySlug)
      ...
      <section className="border-b border-[#e1d6c7] bg-[#fdfaf6]">
        <div className="mx-auto grid max-w-[1320px] sm:grid-cols-2 lg:grid-cols-3">
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 sm:border-r lg:px-8">
            <BedDouble aria-hidden="true" className="h-7 w-7 text-[#a0783e]" strokeWidth={1.2} />
          ...
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 lg:border-r lg:px-8">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-[#3e352a]">Không gian phòng ngủ</p>
          </article>
      ...
      <div className="mx-auto grid max-w-[1320px] gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_330px] lg:py-24">
        <article className="min-w-0">
          ...
          <Link href="/thi-cong-noi-that/khong-gian-phong-khach" className="inline-flex items-center gap-2 border-b border-[#8a7650] pb-2 text-xs font-bold uppercase tracking-[0.08em] text-[#725a36]">
            ...
            Quay lại không gian phòng ngủ
          </Link>
        </article>

        <aside className="h-fit border border-[#ded2c1] bg-[#fdfaf6] p-6 lg:sticky lg:top-8 lg:p-7">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#9a733e]">Tư vấn thiết kế phòng ngủ</p>
          <h2 className="mt-4 font-serif text-3xl leading-tight text-[#30291f]">Cần tư vấn không gian phòng ngủ cho tổ ấm của bạn?</h2>
          ...
          <div className="mt-6 space-y-3 border-y border-[#ded2c1] py-5 text-sm text-[#584f43]">
            <p>
              ...
              <strong className="mt-1 block">Không gian phòng ngủ</strong>
            </p>
            <p>
        ...
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 font-serif text-4xl text-[#30291f]">Bài viết không gian phòng ngủ liên quan</h2>
            </div>
          ...
          <div className="mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {relatedArticles.map((item) => (
              <Link key={item.slug} href={getBedroomSpaceArticleHref(item.slug)} className="group overflow-hidden border border-[#d7c9b6] bg-[#f9f4ed]">
