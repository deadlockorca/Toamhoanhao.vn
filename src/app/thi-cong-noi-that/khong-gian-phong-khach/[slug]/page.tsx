...
import { ArrowLeft, ArrowRight, Check, Phone, Ruler, Sofa, Sparkles } from "lucide-react";

import { ConsultationCta } from "@/components/home/consultation-cta";
import { SiteFooter } from "@/components/site-footer";
import { SiteHeader } from "@/components/site-header";
import {
  getLivingRoomSpaceArticleBySlug,
  getLivingRoomSpaceArticleHref,
  livingRoomSpaceArticles,
} from "@/data/living-room-space-articles";

export function generateStaticParams() {
  return livingRoomSpaceArticles
    .filter((article) => !article.externalHref)
    .map((article) => ({ slug: article.slug }));
}

export async function generateMetadata({
  params,
}: PageProps<"/thi-cong-noi-that/khong-gian-phong-khach/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const article = getLivingRoomSpaceArticleBySlug(slug);
...
export default async function LivingRoomSpaceArticlePage({
  params,
}: PageProps<"/thi-cong-noi-that/khong-gian-phong-khach/[slug]">) {
  const { slug } = await params;
  const article = getLivingRoomSpaceArticleBySlug(slug);
  ...
  const relatedArticles = article.relatedSlugs
    .map(getLivingRoomSpaceArticleBySlug)
          ...
          <div className="relative z-10 flex items-center px-6 py-16 sm:px-10 lg:px-8 lg:py-12">
            <div className="max-w-[600px]">
              <nav aria-label="Điều hướng trang" className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-[#766d60]">
                ...
                <span>/</span>
                <Link href="/thi-cong-noi-that/khong-gian-phong-khach" className="transition hover:text-[#9a733e]">
      ...
      <section className="border-b border-[#e1d6c7] bg-[#fdfaf6]">
        <div className="mx-auto grid max-w-[1320px] sm:grid-cols-2 lg:grid-cols-3">
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 sm:border-r lg:px-8">
            ...
            <p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-[#8c795f]">Nhóm nội dung</p>
          ...
          <article className="border-b border-[#e1d6c7] px-6 py-7 sm:border-b-0 lg:border-r lg:px-8">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-[#3e352a]">Không gian phòng khách</p>
          </article>
          <article className="px-6 py-7 lg:px-8">
            <Sparkles aria-hidden="true" className="h-7 w-7 text-[#a0783e]" strokeWidth={1.2} />
            <p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-[#8c795f]">Đơn vị thực hiện</p>
      ...
      <div className="mx-auto grid max-w-[1320px] gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_330px] lg:py-24">
        <article className="min-w-0">
          <section className="border-b border-[#ded2c1] pb-12">
            ...
            <h2 className="mt-4 font-serif text-4xl leading-tight text-[#30291f] sm:text-5xl">Những điểm nổi bật của bài viết</h2>
                ...
                <div className="mt-6 space-y-5">
                  {section.paragraphs.map((paragraph, paragraphIndex) => {
                    ...
                    return isListItem ? (
                      ...
                      <p key={`${section.title}-${paragraphIndex}`} className="text-base leading-8 text-[#62594d]">{content}</p>
                    );
                  })}
                </div>
                  ...
                  <div className={`mt-8 grid gap-4 ${section.images.length > 1 ? "sm:grid-cols-2" : ""}`}>
                    {section.images.map((image) => (
                      <figure key={image.src} className="overflow-hidden border border-[#ded2c1] bg-white">
                        <Image src={image.src} alt={image.alt} width={800} height={600} sizes="(min-width: 1024px) 38vw, 100vw" className="h-auto w-full object-contain" />
                      </figure>
                    ))}
                  </div>
                ) : null}
          ...
          <Link href="/thi-cong-noi-that/khong-gian-phong-khach" className="inline-flex items-center gap-2 border-b border-[#8a7650] pb-2 text-xs font-bold uppercase tracking-[0.08em] text-[#725a36]">
            <ArrowLeft aria-hidden="true" className="h-4 w-4" />
            Quay lại không gian phòng khách
          </Link>
        </article>

        <aside className="h-fit border border-[#ded2c1] bg-[#fdfaf6] p-6 lg:sticky lg:top-8 lg:p-7">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#9a733e]">Tư vấn thiết kế phòng khách</p>
          <h2 className="mt-4 font-serif text-3xl leading-tight text-[#30291f]">Cần tư vấn không gian phòng khách cho tổ ấm của bạn?</h2>
          <p className="mt-4 text-sm leading-7 text-[#6b6154]">
            Gửi diện tích, phong cách và nhu cầu sử dụng. Đội ngũ Tổ Ấm Hoàn Hảo sẽ trao đổi để tư vấn phương án tối ưu nhất.
          </p>
          <div className="mt-6 space-y-3 border-y border-[#ded2c1] py-5 text-sm text-[#584f43]">
            <p>
              ...
              <strong className="mt-1 block">Không gian phòng khách</strong>
            </p>
            <p>
        ...
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 font-serif text-4xl text-[#30291f]">Bài viết không gian phòng khách liên quan</h2>
            </div>
            <Link href="/thi-cong-noi-that/khong-gian-phong-khach" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.08em] text-[#725a36]">
              Xem tất cả
              <ArrowRight aria-hidden="true" className="h-4 w-4" />
            </Link>
          </div>
          ...
          <div className="mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {relatedArticles.map((item) => (
              <Link key={item.slug} href={getLivingRoomSpaceArticleHref(item.slug)} className="group overflow-hidden border border-[#d7c9b6] bg-[#f9f4ed]">
                <div className="relative aspect-[1.65] overflow-hidden">
                  <Image src={item.image} alt="" fill sizes="(min-width: 1024px) 33vw, 50vw" className="object-cover transition duration-500 group-hover:scale-[1.03]" />
                </div>
                <div className="p-5">
                  <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#9a733e]">{item.label}</p>
