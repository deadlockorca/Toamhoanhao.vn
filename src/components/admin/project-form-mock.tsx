...
import { createProjectDraft, saveProjectDraft } from "@/app/admin/actions";
import {
  ProjectFloorPlanNoteFields,
  ProjectRelatedFields,
  ProjectSpacesFields,
  ProjectStoryFields,
} from "@/components/admin/project-repeat-fields";
import type { Project } from "@/data/projects";

type ProjectFormMockProps = {
  mode: "create" | "edit";
  project?: Project;
};
...
export function ProjectFormMock({ mode, project }: ProjectFormMockProps) {
  const value = project ?? emptyProject;
  const detail = project?.detail;
  const title =
    mode === "create" ? "Thêm dự án mới" : `Chỉnh sửa: ${value.title}`;
  const formAction = project
    ? saveProjectDraft.bind(null, project.slug)
    : createProjectDraft;
      ...
      >
        <div className="space-y-6">
          <Panel title="Thông tin chung">
            <div className="grid gap-5 md:grid-cols-2">
              <Field label="Tên dự án" name="title" defaultValue={value.title} required />
              ...
              <SelectField
                ...
                options={categories}
              />
              <Field label="Vị trí" name="location" defaultValue={value.location} />
              <Field label="Diện tích" name="area" defaultValue={value.area} />
              <Field label="Năm" name="year" defaultValue={value.year} />
          ...
          <Panel title="Thông số & tổng quan">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Field label="Thời gian thi công" name="duration" defaultValue={detail?.duration} />
              <Field label="Hạng mục" name="scope" defaultValue={detail?.scope} />
            </div>
            <Field
          ...
          <Panel title="Không gian nổi bật">
            <ProjectSpacesFields detail={detail} />
          </Panel>

          <Panel title="Câu chuyện thiết kế">
            <ProjectStoryFields detail={detail} />
          </Panel>

          <Panel title="Mặt bằng">
            <Field
              label="Ảnh mặt bằng"
              name="floorPlanImage"
            ...
            />
            <div className="mt-5">
              <ProjectFloorPlanNoteFields detail={detail} />
            </div>
          </Panel>

          <Panel title="Dự án liên quan">
            <ProjectRelatedFields detail={detail} />
          </Panel>
        </div>
        ...
        <aside className="space-y-6">
          ...
          <Panel title="Ảnh đại diện">
            <div className="relative aspect-[1.36] overflow-hidden border border-[#ded4c4] bg-white">
              {value.thumbnail ? (
                <Image
                  src={value.thumbnail}
                  alt={value.title || "Ảnh dự án"}
                  fill
                  sizes="340px"
          ...
          <Panel title="Cấu trúc detail">
            <div className="space-y-3 text-sm text-[#62584b]">
              {[
                ...
                { icon: ListPlus, text: "Không gian nổi bật" },
                { icon: ListPlus, text: "Câu chuyện thiết kế" },
                { icon: ListPlus, text: "Mặt bằng" },
                { icon: ListPlus, text: "Dự án liên quan" },
              ].map((item) => {
