import { Building2, Gem, House, Ruler } from "lucide-react";

import type { DesignSample } from "@/data/design-samples";

type DesignDetailMetricsProps = {
  sample: DesignSample;
};

const metricIcons = [Ruler, Gem, Building2, House];

export function DesignDetailMetrics({ sample }: DesignDetailMetricsProps) {
  const metrics = sample.detail?.metrics ?? [
    { label: "Diện tích", value: sample.area ?? "Theo hiện trạng" },
    { label: "Phong cách", value: sample.style },
    { label: "Phù hợp", value: sample.type },
    { label: "Mức đầu tư", value: "Theo nhu cầu" },
  ];
