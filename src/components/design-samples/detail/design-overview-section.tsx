...
type DesignOverviewSectionProps = {
  sample: DesignSample;
};

const infoIcons = [Home, Ruler, BedDouble, Bath, Palette, Building2, Gem];

export function DesignOverviewSection({ sample }: DesignOverviewSectionProps) {
    ...
    <section className="bg-[#f7f1e9] px-5 py-16 sm:px-8">
      <div className="mx-auto grid max-w-[1120px] gap-10 lg:grid-cols-[44%_56%]">
        ...
        <div className="border border-[#ded4c4] bg-[#fbf7f1]/72 px-7 py-6">
          {rows.map((row, index) => {
            const Icon = infoIcons[index] ?? Home;
